...
const EventList: React.FC<IProps> = ({events = [], setEventList, user}) => {
  const navigation =
    useNavigation<NativeStackNavigationProp<{ Details: { eventId: string } }>>();
    ...
    const fetchEventList = (dispatch: (eventList: IEvent[]) => void) => {
        axios.get(`/events`)
            ...
            .catch(err => {
                console.log(err);
                Alert.alert("Błąd", "Nie udało się pobrać listy wydarzeń. ", [{text: "OK"}]);
                return;
            })
    }
        ...
        {(events || []).map((entry, i) => {
          return (
            <EventCard
              event={entry}
              key={i}
              click={() => navigation.navigate("Details", { eventId: entry.id })}
            />
          );
...
const mapDispatchToProps = (dispatch: any) => ({
    setEventList: (eventList: IEvent[]) => dispatch(setEventListAction(eventList)),
});
